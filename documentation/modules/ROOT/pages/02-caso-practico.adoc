...
Vemos que el mensaje de bienvenida ha cambiado siendo **Bienvenido a una maquina desplegada con bootc-container-vm v2**. Lo que ha pasado es que el timer sigue activo y ha detectado que en el registry existe una nueva versión ya que al hacer el rollback hemos vuelto a la antigua, y procede a actualizarla. 
...
Si se decidiera utilizar el timer para la autoactualización entonces al hacer el rollback se cambiaría en el registry el tag **latest** para que apuntará a la versión del contenedor que se va a hacer rollback para que, de esta forma, la información del registry sea congruente con la configuración en producción.
=====
